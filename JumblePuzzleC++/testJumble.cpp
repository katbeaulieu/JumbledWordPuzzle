...
void testJumble() {
	...
	//test the exception, case when the length of the word is less than 3
	try{
		JumblePuzzle shortWord("at","easy");
	}
	catch(BadJumbleException &e){
		cout << e.what() << endl;
		cout << " " << endl;
	}
	//test exception when the length of word is greater than 10
	try{
		JumblePuzzle longWord("supercalifragilisticexpialidocious", "medium");
	}
	catch (BadJumbleException &e){
		cout << e.what() << endl;
		cout << " " << endl;
	...
	//text exception when the difficulty supplied is not legitimate.
	try{
		JumblePuzzle weirdDifficulty("hellothere", "very");
	...
	//test the accessors, so generate a puzzle and get the direction and starting row
	//and col position to easily find the hidden word.
	//Also test accessors for difficulty and size of the puzzle.
	JumblePuzzle testPuzzle("MEOW","easy");
	int colPos = testPuzzle.getColPos();
	int rowPos = testPuzzle.getRowPos();
	char direction = testPuzzle.getDirection();
	int difficulty = testPuzzle.getDifficulty();
	int size = testPuzzle.getSize();
	cout << "Col Pos:" << colPos << endl;
	cout << "Row Pos:" << rowPos << endl;
	cout << "Direction: " << direction << endl;
	cout << "Difficulty: " << difficulty << endl;
	cout << "Size: " << size << endl;
	showJumble(testPuzzle.getJumble(),testPuzzle.getSize());
	cout << " " << endl;
